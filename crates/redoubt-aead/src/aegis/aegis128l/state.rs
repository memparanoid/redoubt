...
pub unsafe fn encrypt(
    ...
    for block in aad_iter.by_ref() {
        let mut m0 = Intrinsics::load(
            block[..16]
                .try_into()
                .expect("Infallible: BLOCK_SIZE is 32, so [..16] is exactly 16 bytes"),
        );
        let mut m1 = Intrinsics::load(
            block[16..]
                .try_into()
                .expect("Infallible: BLOCK_SIZE is 32, so [16..] is exactly 16 bytes"),
        );
    ...
    if !aad_remainder.is_empty() {
        ...
        let mut m0 = Intrinsics::load(
            padded[..16]
                .try_into()
                .expect("Infallible: padded is [u8; BLOCK_SIZE], so [..16] is exactly 16 bytes"),
        );
        let mut m1 = Intrinsics::load(
            padded[16..]
                .try_into()
    ...
    for block in data_iter.by_ref() {
        ...
        c0.store(
            (&mut block[..16])
                .try_into()
                .expect("Infallible: mutable slice is exactly 16 bytes"),
        );
        c1.store(
            (&mut block[16..])
                .try_into()
    ...
    if !data_remainder.is_empty() {
        ...
        c0.store(
            (&mut ct_buf[..16])
                .try_into()
                .expect("Infallible: ct_buf is [u8; BLOCK_SIZE], so [..16] is exactly 16 bytes"),
        );
        c1.store(
            (&mut ct_buf[16..])
                .try_into()
    ...
    u64_to_le(
        ...
        (&mut len_block[..8])
            .try_into()
            .expect("Infallible: len_block is [u8; 16], so [..8] is exactly 8 bytes"),
    );
    u64_to_le(
        &mut msg_bits,
        (&mut len_block[8..])
...
pub unsafe fn decrypt(
    ...
    for block in data_iter.by_ref() {
        ...
        // Load ciphertext
        let mut ct0 = Intrinsics::load(
            block[..16]
                .try_into()
                .expect("Infallible: BLOCK_SIZE is 32, so [..16] is exactly 16 bytes"),
        );
        let mut ct1 = Intrinsics::load(
            block[16..]
                .try_into()
                .expect("Infallible: BLOCK_SIZE is 32, so [16..] is exactly 16 bytes"),
        );
    ...
    if !data_remainder.is_empty() {
        ...
        pt0.store(
            (&mut pt_buf[..16])
                .try_into()
                .expect("Infallible: pt_buf is [u8; BLOCK_SIZE], so [..16] is exactly 16 bytes"),
        );
        pt1.store(
            (&mut pt_buf[16..])
                .try_into()

...
//           This allows zeroization of ALL registers without save/restore overhead.
//
// Zeroizes everything except control flow registers:
//   - v0-v31   : ALL SIMD/NEON registers
//   - x0-x17   : ALL caller-saved GPR
//   - x19-x28  : ALL callee-saved GPR (safe - we don't use them)
//
// Never zeroized (control flow):
//   - x18      : Platform register (reserved by OS)
//   - x29 (FP) : Frame pointer
//   - x30 (LR) : Link register (needed for ret)
//   - sp       : Stack pointer (needed for return sequence)
//
.macro AEGIS_ZEROIZE_ALL
    // === SIMD/NEON: Zeroize ALL v0-v31 ===
    movi v0.16b, #0
    movi v1.16b, #0
    movi v2.16b, #0
    movi v3.16b, #0
    ...
    // === GPR: Zeroize x0-x17 (caller-saved) ===
    mov x0, xzr
    mov x1, xzr
    mov x2, xzr
    mov x3, xzr
    ...
    // === GPR: Zeroize x19-x28 (callee-saved - safe because we don't use them) ===
    mov x19, xzr
    mov x20, xzr
    mov x21, xzr
    mov x22, xzr

...
//           This allows zeroization of ALL caller-saved registers.
//
// Zeroizes ONLY caller-saved registers (the ones we use):
...
//
// Never zeroized (must preserve for ABI):
//   - x18      : Platform register (reserved by OS)
//   - x19-x28  : Callee-saved GPR (caller expects these preserved!)
//   - x29 (FP) : Frame pointer
//   - x30 (LR) : Link register (needed for ret)
//   - sp       : Stack pointer
//   - v8-v15   : Callee-saved SIMD (caller expects these preserved!)
//
.macro AEGIS_ZEROIZE_ALL
    // === SIMD/NEON: Zeroize ONLY caller-saved v0-v7, v16-v31 ===
    movi v0.16b, #0
    movi v1.16b, #0
    movi v2.16b, #0
    movi v3.16b, #0
    ...
    movi v6.16b, #0
    movi v7.16b, #0
    // SKIP v8-v15 (callee-saved - caller expects preserved)
    movi v16.16b, #0
    movi v17.16b, #0
    movi v18.16b, #0
    movi v19.16b, #0
    ...
    // === GPR: Zeroize ONLY caller-saved x0-x17 ===
    mov x0, xzr
    mov x1, xzr
    mov x2, xzr
    mov x3, xzr
    ...
    mov x16, xzr
    mov x17, xzr
    // SKIP x19-x28 (callee-saved - caller expects preserved)
.endm
...
//
// Register allocation (ALL caller-saved - no frame needed):
//   v0-v7   = AEGIS state S0-S7
//   v16     = key (caller-saved, zeroized before return)
//   v17     = nonce (caller-saved, zeroized before return)
//   v18     = C0 constant
//   v19     = C1 constant
//   v20-v31 = temporaries for keystream, plaintext, ciphertext, zero vector
//   x8      = key pointer
//   x9      = nonce pointer
//   x10     = AAD pointer (also used as temp)
//   x11     = AAD/plaintext remaining length (also used as temp)
//   x12     = plaintext pointer
//   x13     = plaintext length
//   x14     = ciphertext pointer
//   x15     = tag pointer
//   x0-x7   = temporaries for constant loading, loop counters
//
...
FUNC(aegis128l_encrypt):
    // Prologue: NO callee-saved registers needed (we use only caller-saved)
    // No frame needed for register saves - only for partial block spills (32 bytes max)

    // Save parameters to caller-saved registers (x8-x15)
    mov x8, x0                       // key pointer
    mov x9, x1                       // nonce pointer
    mov x10, x2                      // AAD pointer
    mov x11, x3                      // AAD length
    mov x12, x4                      // plaintext pointer
    mov x13, x5                      // plaintext length
    mov x14, x6                      // ciphertext pointer
    mov x15, x7                      // tag pointer

    // === Phase 1: Initialization (inline, no function call) ===
    // Load key and nonce into caller-saved SIMD registers v16-v17
    ld1 {v16.16b}, [x8]              // v16 = key
    ld1 {v17.16b}, [x9]              // v17 = nonce

    // Load constants C0 and C1 into v18-v19 (using x0 as temp)
#if defined(__APPLE__)
    ...
    // Use x6-x7 for AAD loop to preserve x10-x11 for finalization
    mov x6, x10                      // x6 = AAD pointer
    mov x7, x11                      // x7 = AAD remaining

.Laad_full_blocks:
    cmp x7, #32                      // Check if at least 32 bytes remain
    b.lt .Laad_partial               // If less, handle partial block

    // Load full 32-byte AAD block
    ld1 {v12.16b}, [x6], #16         // M0 = first 16 bytes
    ld1 {v13.16b}, [x6], #16         // M1 = second 16 bytes

    // Update state with AAD
    AEGIS_UPDATE v12, v13

    sub x7, x7, #32                  // Remaining -= 32
    b .Laad_full_blocks

.Laad_partial:
    // Handle partial AAD block (1-31 bytes remaining in x7)
    cbz x7, .Laad_done               // If 0 bytes, skip
...
// ║═══════════════════════════════════════════════════════════════════════
    ...
    // Copy partial AAD bytes to stack buffer (using x0-x2 as temporaries)
    mov x0, sp                       // x0 = buffer pointer
    mov x1, x7                       // x1 = bytes to copy (x7 has remaining)
.Laad_spill_copy_loop:
    cbz x1, .Laad_spill_copy_done
    ldrb w2, [x6], #1                // Load byte from AAD (x6 = AAD pointer)
    strb w2, [x0], #1                // Store to buffer
    sub x1, x1, #1
...
.Laad_done:
    ...
    // Use x1-x2 for loop to preserve x10-x13 (needed for finalization)
    mov x1, x12                      // x1 = plaintext pointer (from x12)
    mov x2, x13                      // x2 = plaintext remaining bytes (from x13)

.Lenc_full_blocks:
    cmp x2, #32                      // Check if at least 32 bytes remain
    ...
    // Load plaintext
    ld1 {v14.16b}, [x1], #16         // plaintext block 0
    ld1 {v15.16b}, [x1], #16         // plaintext block 1
    ...
    sub x2, x2, #32                  // Remaining -= 32
    b .Lenc_full_blocks

.Lenc_partial:
    // Handle partial plaintext block (1-31 bytes remaining in x2)
    cbz x2, .Lfinalize               // If 0 bytes, skip to finalization
...
// ║═══════════════════════════════════════════════════════════════════════
    ...
    // Copy partial plaintext to buffer (using x3-x5 as temporaries)
    mov x3, sp                       // x3 = buffer pointer
    mov x4, x2                       // x4 = bytes to copy (x2 has remaining)
.Lenc_spill_copy_pt_loop:
    cbz x4, .Lenc_spill_copy_pt_done
    ldrb w5, [x1], #1                // Load byte from plaintext (x1 = pt pointer)
    strb w5, [x3], #1                // Store to buffer
    sub x4, x4, #1
...
.Lenc_spill_copy_pt_done:
    ...
    // Copy only valid ciphertext bytes to output (using x3-x5 as temporaries)
    mov x3, sp                       // x3 = buffer pointer
    mov x4, x2                       // x4 = bytes to copy
.Lenc_spill_copy_ct_loop:
    cbz x4, .Lenc_spill_copy_ct_done
    ldrb w5, [x3], #1                // Load byte from buffer
    strb w5, [x14], #1               // Store to ciphertext output
    sub x4, x4, #1
...
.Lfinalize:
    ...
    // aad_len is in x11, plaintext_len is in x13 - use x0-x1 as temps
    lsl x0, x11, #3                  // x0 = aad_bits (64-bit)
    lsl x1, x13, #3                  // x1 = msg_bits (64-bit)

    // Build 128-bit block: [aad_bits (low 64) || msg_bits (high 64)]
    // Store to v28 as [aad_bits, msg_bits] in little-endian
    fmov d28, x0                     // d28 = aad_bits (lower 64 bits of v28)
    mov v28.d[1], x1                 // v28.d[1] = msg_bits (upper 64 bits of v28)
    ...
    // 🗑️❗ CATASTROPHIC REGISTER ZEROIZATION
    AEGIS_ZEROIZE_ALL

    // No stack frame to restore - we used only caller-saved registers
    ret

...
/// How many capacities an equidistant set spans.
///
/// What consumes one is a doubly nested loop that clones, encodes, decodes and
/// probes zeroization on every pass, so the cost is quadratic in it: at 250,
/// ~62,000 inner rounds with payloads that grow with the outer index.

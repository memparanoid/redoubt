...
fn run_test_case(tc: &TestCase) -> Result<(), String> {
    ...
    // Check if size is too large (invalid test)
    if tc.size > MAX_OUTPUT_LEN {
        return match tc.result {
            TestResult::Invalid => Ok(()), // Expected to fail
            _ => Err(format!(
                "tc_id {}: size {} exceeds MAX_OUTPUT_LEN but marked as valid",
                tc.tc_id, tc.size
            )),
        };
    }
    ...
    let result = hkdf(&ikm, &salt, &info, &mut out);
...
fn test_wycheproof_size_too_large() {
    ...
    for tc in vectors.iter() {
        ...
        assert!(
            tc.size > MAX_OUTPUT_LEN,
            "tc_id {}: SizeTooLarge flag but size {} <= MAX_OUTPUT_LEN {}",
            tc.tc_id,
            tc.size,
            MAX_OUTPUT_LEN
        );

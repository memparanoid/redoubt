...
//!
//! `rust` and `asm` are the implementations; `seam` is what chooses between
...
//! and anything asked of every backend goes through `seam`.
...
///
/// The backends are the same code where the target has no assembly, and a pair
/// of cases that found them agreeing there would have proved nothing about

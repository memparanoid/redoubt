...
use memcode::{MemBytesRequired, MemCodec, MemEncodeBuf, MemEncode, MemDecode};
...
fn bench_encode(c: &mut Criterion) {
    ...
    // --- u8 ---
    let data_u8 = DataU8 { values: vec![0xAB; count] };
    ...
    // --- u16 ---
    let data_u16 = DataU16 { values: vec![0xABCD; count] };
    ...
    // --- u32 ---
    let data_u32 = DataU32 { values: vec![0xDEADBEEF; count] };
    ...
    // --- u64 ---
    let data_u64 = DataU64 { values: vec![0xDEADBEEFCAFEBABE; count] };
...
fn bench_decode(c: &mut Criterion) {
    ...
    group.bench_with_input(BenchmarkId::new("bincode/u8", count), &bincode_u8, |b, enc| {
    ...
    group.bench_with_input(BenchmarkId::new("bincode/u16", count), &bincode_u16, |b, enc| {
        b.iter(|| black_box(bincode::deserialize::<DataU16>(enc).unwrap()));
    });

    group.bench_with_input(BenchmarkId::new("memcode/u16", count), &buf_u16, |b, buf| {
        b.iter_batched(
            || buf.as_slice().to_vec(),
            |mut bytes| {
    ...
    group.bench_with_input(BenchmarkId::new("bincode/u32", count), &bincode_u32, |b, enc| {
        b.iter(|| black_box(bincode::deserialize::<DataU32>(enc).unwrap()));
    });

    group.bench_with_input(BenchmarkId::new("memcode/u32", count), &buf_u32, |b, buf| {
        b.iter_batched(
            || buf.as_slice().to_vec(),
            |mut bytes| {
    ...
    group.bench_with_input(BenchmarkId::new("bincode/u64", count), &bincode_u64, |b, enc| {
        b.iter(|| black_box(bincode::deserialize::<DataU64>(enc).unwrap()));
    });

    group.bench_with_input(BenchmarkId::new("memcode/u64", count), &buf_u64, |b, buf| {
        b.iter_batched(
            || buf.as_slice().to_vec(),
            |mut bytes| {
...
fn bench_mixed(c: &mut Criterion) {
    ...
    group.bench_with_input(BenchmarkId::new("decode/bincode", count), &bincode_encoded, |b, enc| {
...
criterion_group!(benches, bench_encode, bench_decode, bench_mixed);

...
//! That both backends answer what the standard library's check answers, which
//! is the oracle: an implementation from outside this workspace.
...
fn test_is_utf8_answers_as_the_standard_library_on_nothing(#[case] backend: Backend) {
    agrees(backend, &[]);
}
...
#[case::rust(Backend::Rust)]
#[case::auto(Backend::Auto)]
fn test_is_utf8_answers_as_the_standard_library_on_every_byte(#[case] backend: Backend) {
    for first in 0..=u8::MAX {
        agrees(backend, &[first]);
    }
}
...
#[case::rust(Backend::Rust)]
#[case::auto(Backend::Auto)]
fn test_is_utf8_answers_as_the_standard_library_on_every_pair(#[case] backend: Backend) {
    for first in 0..=u8::MAX {
...
#[case::rust(Backend::Rust)]
#[case::auto(Backend::Auto)]
fn test_is_utf8_answers_as_the_standard_library_on_every_triple(#[case] backend: Backend) {
    for first in 0..=u8::MAX {
        for second in 0..=u8::MAX {
            for third in 0..=u8::MAX {
                agrees(backend, &[first, second, third]);
            }
        }
    }
}
...
#[case::rust(Backend::Rust)]
#[case::auto(Backend::Auto)]
fn test_is_utf8_answers_as_the_standard_library_on_every_four_byte_lead(#[case] backend: Backend) {
    for first in 0xF0..=u8::MAX {
        for second in 0..=u8::MAX {
            for third in EDGES {
                for fourth in EDGES {
                    agrees(backend, &[first, second, third, fourth]);
                }
            }
        }
    }

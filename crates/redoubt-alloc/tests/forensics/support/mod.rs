...
/// The element of a container measured one element at a time: as wide as the
/// needle, so what a move leaves is a run the sweep can read.
pub(crate) type Block = [u8; SECRET.len()];

/// How many blocks make up `of` bytes.

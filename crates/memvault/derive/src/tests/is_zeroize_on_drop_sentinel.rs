...
// === === === === === === === === === ===
// is_zeroize_on_drop_sentinel_type() tests
// === === === === === === === === === ===

#[test]
fn test_zeroize_on_drop_sentinel_simple_path() {
    let ty: Type = parse_quote!(ZeroizeOnDropSentinel);
    assert!(is_zeroize_on_drop_sentinel_type(&ty));
}

#[test]
fn test_zeroize_on_drop_sentinel_qualified_path() {
    let ty: Type = parse_quote!(redoubt_zero::ZeroizeOnDropSentinel);
    assert!(is_zeroize_on_drop_sentinel_type(&ty));
}

#[test]
fn test_zeroize_on_drop_sentinel_fully_qualified_path() {
    let ty: Type = parse_quote!(::redoubt_zero::ZeroizeOnDropSentinel);
    assert!(is_zeroize_on_drop_sentinel_type(&ty));
}

#[test]
fn test_non_zeroize_on_drop_sentinel_type_returns_false() {

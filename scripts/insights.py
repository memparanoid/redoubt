...
def count_tests():
    """How many tests the workspace has, asked of nextest rather than run.

    `list` builds the binaries and enumerates them, so the count includes the
    integration tests that `--lib` leaves out and the ignored ones that a run
    would report as skipped.
    """
    output = run_cmd("cargo nextest list --workspace --message-format json 2>/dev/null")

    try:
        return json.loads(output)['test-count']

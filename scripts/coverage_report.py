...
"""
The page over Redoubt's coverage, rendered from what is already on disk.

Nothing is measured here. `./scripts/coverage.sh` runs the crates and writes
`<crate>/html/index.html` for each; this reads those, adds them up, and writes
the index and the public report `insights.py` takes its badge from.

Usage:
    ./scripts/coverage.py               # render, open it
